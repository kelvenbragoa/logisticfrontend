...
const deleteData = () => {
    ...
    axios
        .delete(`${baseURL}/trips/${dataIdBeingDeleted.value}`)
        .then(() => {
            ...
            <DataTable :value="retriviedData.data" v-model:expandedRows="expandedRows" tableStyle="min-width: 50rem">
                <template #header>
                    <div class="flex flex-wrap align-items-center justify-content-between gap-2">
                        <span class="text-xl text-900 font-bold">Viagens</span>
                ...
                <Column field="destination.departure" sortable header="Partida"></Column>
                <Column field="destination.destination" sortable header="Destino"></Column>
                <Column field="vehicle.name" sortable header="Veículo">
                    <template #body="slotProps">
                        {{ slotProps.data.vehicle.name }} -  {{ slotProps.data.vehicle.plate }} 
                    </template>
                </Column>
                <Column field="trela.name" sortable header="Trela"></Column>
                <Column field="driver.name" sortable header="Motorista"></Column>
                <Column field="tripstatus.name" sortable header="Status">
                    <template #body="slotProps">
                        <Tag severity="warning" v-if="slotProps.data.trip_status_id == 1">{{ slotProps.data.tripstatus.name }}</Tag>
                        <Tag severity="info" v-if="slotProps.data.trip_status_id == 2">{{ slotProps.data.tripstatus.name }}</Tag>
                ...
                <Column header="Ações">
                    <template #body="slotProps">
                        <router-link :to="'/trips/' + slotProps.data.id + '/edit'"><i class="pi pi-file-edit"></i></router-link> | <router-link :to="'/trips/' + slotProps.data.id"><i class="pi pi-eye"></i></router-link> |
                ...
                <template #footer> No total são {{ retriviedData.data ? retriviedData.total : 0 }} Viagens. </template>
            </DataTable>

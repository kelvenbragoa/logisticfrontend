...
const getData = () => {
    axios
        .get(`${baseURL}/drivers/${router.currentRoute.value.params.id}`)
        .then((response) => {
...
<template>
    <div className="card" v-if="!isLoadingDiv">
        <div class="col-12">
            ...
            <p>Detalhes da Motorista</p>
            <p><strong>Nome:</strong> {{ retriviedData.name }}</p>
            <p><strong>Carta de Condução:</strong> {{ retriviedData.license }}</p>
            <p><strong>Telefone:</strong> {{ retriviedData.mobile }}</p>
            <!-- <p><strong>Província:</strong> {{ retriviedData.mobile }}</p> -->
        </div>
    </div>

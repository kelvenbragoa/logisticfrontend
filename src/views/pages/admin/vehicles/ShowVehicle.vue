...
const getData = () => {
    axios
        .get(`${baseURL}/vehicles/${router.currentRoute.value.params.id}`)
        .then((response) => {
...
<template>
    <div className="card" v-if="!isLoadingDiv">
        <div class="col-12">
            ...
            <p>Detalhes da Veículo</p>
            <p><strong>Nome:</strong> {{ retriviedData.name }}</p>
            <p><strong>Matricula:</strong> {{ retriviedData.plate }}</p>
            <p><strong>Capacidade Máxima:</strong> {{ retriviedData.max_load }}</p>
